      ...
      <el-card class="box-card" shadow="never">
        ...
        <div class="topic-intro">
          ...
          <div class="topic-description">
            ...
            <span
              >值不值得买，是一件很头痛的事情。树洞评测，帮你try and before you
              buy。</span
            >
          </div>
        </div>
  ...
  methods: {
    ...
    async followUnfollow() {
      if (!this.currentUserId) {
        ...
        const response = await followUnfollow("GeekToBuy", this.currentUserId);
    ...
    async fetchPosts(pageSize) {
      const _posts = await fetchPostsByCategory("GeekToBuy", pageSize);
    ...
    async categoryStatsInit() {
      ...
      } = await fetchCategoryStats("GeekToBuy");
...
.topic-wallpaper .wallpaper {
  background-image: url("~@/assets/treesays_buy.png");
