...
const fetchPostsByCategory = (whichCategory, pageNum = 0, pageSize = 7) => {
  ...
  return query.find(
    query
      ...
      .equalTo("category", whichCategory)
      .limit(pageSize)
...
const followUnfollow = (whichCategory, userId) => {
  ...
  return new AV.Query("CagegoryStats").get(categoryId).then(async data => {
    const target = AV.Object.createWithoutData("CagegoryStats", categoryId);
    const index = data.attributes.followers.indexOf(userId);
    index < 0
      ? data.attributes.followers.push(userId)
      : data.attributes.followers.splice(index, 1);
...
const isUserFollowedCategory = (whichCategory, userId) => {
  return new AV.Query("CagegoryStats")
    .get(categoryMapping[whichCategory])
    .then(data => {
...
const fromPostsAPIResponse = post => {
  ...
  return {
    ...
    id,
    show,
    createdAt: moment(createdAt).toNow()
  };
};
export {
  fetchCategoryStats,
